...
class BaseRunner(object):
	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
		...
		is_ce_loss=hasattr(model,'loss_type') and model.loss_type.upper()=='CE'
		indices, arange_idx=None, None
		...
		for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
			...
			if not is_ce_loss:
				...
				indices = torch.argsort(torch.rand(*item_ids.shape,device=device), dim=-1)
				arange_idx=torch.arange(item_ids.shape[0],device=device).unsqueeze(-1)						
				batch['item_id'] = item_ids[arange_idx, indices]
			...
			if not is_ce_loss:
				...
				if len(prediction.shape)==2: # only for ranking tasks
					...
					restored_prediction[arange_idx, indices] = prediction   
